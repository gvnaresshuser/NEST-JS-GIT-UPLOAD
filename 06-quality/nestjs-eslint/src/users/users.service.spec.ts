...
describe('UsersService - Step 1', () => {
  ...
  it('should create user', async () => {
    ...
    (db.insert as jest.Mock).mockReturnValue({
      values: jest.fn().mockReturnValue({
        returning: jest.fn().mockResolvedValue([{ ...mockUser, password: 'hashed_password' }]),
      }),
    });
  ...
  it('should throw error if email not found', async () => {
    ...
    await expect(service.login('wrong@example.com', '123456')).rejects.toThrow(
  ...
  it('should throw error if password not set', async () => {
    ...
    await expect(service.login('john@example.com', '123456')).rejects.toThrow('Password not set');
  });
  //----------------------------------
