...
export class UsersController {
  ...
  getProfile(@GetUser() user: any) {
    console.warn("[ROUTE] - @Get('profile')");
    return {
  ...
  async login(@Body() body: LoginDto) {
    console.warn('[ROUTE] - @Post(login)');
    console.warn('Email:', body.email);
    console.warn('Password:', body.password);
  ...
  findAllFilter(@Query('search') search?: string) {
    console.warn('[ROUTE] - GET /users/search?search=');
  ...
  @UseGuards(JwtAuthGuard)
  findAllSecure() {
    console.warn('[ROUTE] - GET /users/secure (JWT)');
    return this.usersService.findAll();
  }
  ...
  findAllSecureFilter(@Query('search') search?: string) {
    console.warn('[ROUTE] - GET /users/secure/search');
    return this.usersService.findAllFiltered(search);
  }
  ...
  findOneCustom(@Param('id', ParseIntCustomPipe) id: number) {
    console.warn('[ROUTE] - Custom ParseIntCustomPipe');
  ...
  findOne(@Param('id', ParseIntPipe) id: number) {
    console.warn('[ROUTE] - GET /users/:id');
    return this.usersService.findOne(id);
  }
  ...
  remove(@Param('id', ParseIntPipe) id: number) {
    console.warn('[ROUTE] - @Delete()-remove(id)');
