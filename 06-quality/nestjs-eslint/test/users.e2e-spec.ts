...
describe('UsersController (E2E)', () => {
  ...
  it('GET /users/secure → requires token', async () => {
    ...
    // ✅ Login to get token
    const loginRes = await request(app.getHttpServer()).post('/users/login').send({
      email: 'secure@mail.com',
      password: '1234',
    });
    ...
    await request(app.getHttpServer()).get('/users/secure').expect(401);
  ...
  it('DELETE /users/:id', async () => {
    ...
    await request(app.getHttpServer()).delete(`/users/${createRes.body.id}`).expect(200);
  });
});
/*
//START STUDIO:
